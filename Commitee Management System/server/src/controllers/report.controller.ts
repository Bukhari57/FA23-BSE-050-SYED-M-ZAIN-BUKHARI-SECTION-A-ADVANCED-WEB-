...
const buildFinancialSummary = async (start: Date, end: Date) => {
...
export const generateReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { type, format, startDate, endDate, committeeId } = req.body as {
  ...
  if (format === 'pdf') {
    ...
    if (committee) doc.text(`Committee: ${committee.name}`);
    doc.moveDown();
