  ...
  return (
    <div className="animate-fade-in flex flex-col gap-4">
      <div className="hidden sm:block">
        ...
        <p className="page-subtitle">Chat with your assigned doctor</p>
      </div>

      <div className="card overflow-hidden flex h-[calc(100dvh-72px)] sm:h-[calc(100dvh-90px)] md:h-[70vh]">

        {/* Contact list */}
        <div className={`${inChat ? 'hidden' : 'flex'} md:flex w-full md:w-72 border-r border-slate-100 dark:border-slate-700 flex-col flex-shrink-0`}>
          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Your Doctor</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {contacts.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full px-6 text-center">
                ...
                <p className="text-sm font-medium text-slate-500">No doctor assigned</p>
                <p className="text-xs text-slate-400 mt-1">Ask an admin to link you to a doctor</p>
              </div>
            ) : (
              contacts.map((c) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-4 text-left hover:bg-slate-50 dark:hover:bg-slate-700/50 active:bg-slate-100 transition-colors border-b border-slate-50 dark:border-slate-700/30 ${
                    selected?.id === c.id ? 'bg-blue-50 dark:bg-blue-900/20 border-l-4 border-l-blue-600' : ''
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate">Dr. {c.name}</p>
        ...
        {/* Chat area */}
        <div className={`${inChat ? 'flex' : 'hidden'} md:flex flex-1 flex-col min-w-0`}>
          {!selected ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              <MessageIcon className="w-14 h-14 text-slate-200 mb-4" />
              <p className="text-slate-500 dark:text-slate-400 font-medium">Select a contact to start chatting</p>
            </div>
          ) : (
            <>
              <div className="px-3 sm:px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={() => setMobileView('contacts')}
                  className="md:hidden p-2 -ml-1 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 active:bg-slate-200 transition-colors flex-shrink-0"
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-slate-900 dark:text-slate-100 text-sm truncate">Dr. {selected.name}</p>
                    ...
                    return (
                      <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[78%] sm:max-w-sm lg:max-w-md px-3 sm:px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                          ...
                          <p className={`text-[11px] mt-1 text-right ${isMe ? 'text-blue-200' : 'text-slate-400 dark:text-slate-400'}`}>
              ...
              <form onSubmit={handleSend} className="px-3 sm:px-4 py-3 border-t border-slate-100 dark:border-slate-700 flex items-center gap-2 flex-shrink-0">
                <input
                  ref={inputRef}
                  type="text"
                  value={text}
